...
if (window.isc && isc.version != "SNAPSHOT_v13.0d_2020-07-07/LGPL Deployment" && !isc.DevUtil) {
    isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
        + "SmartClient version '" + isc.version + "' and additional modules from 'SNAPSHOT_v13.0d_2020-07-07/LGPL Deployment'. Mixing resources from different "
...
isc.CalendarView.addProperties({
    ...
    setChosenDate : function (newDate) {
        ...
        var chosenDate = isc.DateUtil.getLogicalDateOnly(newDate);
    ...
    rebuild : function (refreshData) {
        ...
        else this.refreshEvents("rebuild");
    },
...
// cell hovers
    ...
    asyncGetPrintHTML : function (printProperties, callback) {
        ...
        if (this.showHeader) {
            ...
            //this._headerPrintHeight = isc.Canvas.measureContent(headerHTML, "printHeader", true, true);
    ...
    eventDragCanvasDefaults: {
        ...
        dragRepositionMove : function () {
            ...
            if (eventCol != props._currentCol) {
                if (view.isDayView() || view.isWeekView()) {
                    if (view.hasLanes() && !cal.canEditEventLane(event, view)) {
                        // lanes in dayView
        ...
        dragRepositionStop : function () {
            ...
            if (cal.getEventStartDate(event).getTime() == cal.getEventStartDate(newEvent).getTime()) {
                if (!view.hasLanes() || event[cal.laneNameField] == newEvent[cal.laneNameField]) {
        ...
        dragResizeStart : function () {
            ...
            if (isTimeline) {
                ...
                props = {
                    _useLanes: view.hasLanes(),
                    _useSublanes: cal.useSublanes,
                    _previousLeft: colLeft + (hasLanes && sublane ? sublane.left : 0),
                    _previousRight: canvas.getLeft() + eventWidth,
        ...
        dragResizeMove : function () {
            var canvas = this.eventCanvas,
                ...
                utils = isc.DateUtil
            ;
            var mouseSnap = view.getSnapData();
            var snapDate;
            if (props._bottomDrag) {
                ...
                //endDate = isc.DateUtil.createDatetime(endDate);
    ...
    resized : function (deltaX, deltaY, reason ) {
        ...
        if (this.renderEventsOnDemand) {
            if (!this._updatingRecordComponents && this.isDrawn()) {
                if (this._drawnCanvasList) {
                    this._drawnCanvasList.setProperty("needsResize", true);
                }
    ...
    findEventsInRange : function (startDate, endDate, lane, data) {
        var cal = this.calendar,
            range = {},
            useLane = lane != null && this.hasLanes()
        ;
    ...
    sizeEventCanvas : function (canvas, forceRedraw, calledFrom) {
        ...
        if (this.shouldShowColumnLayouts()) {
            this.logInfo("sizeEventCanvas() called from " + calledFrom + " whille " +
        ...
        if (isTimeline) {
            ...
            var newStart = isc.DateUtil.getLogicalDateOnly(startDate);

            var colNum;
            if (this.isDayView()) {
                if (this.hasLanes()) colNum = laneIndex;
                else colNum = 0;
            } else {
            ...
            if (startDate.getTime() < colDate.getTime()) {
            ...
            if (eHrs == null) {
                ...
                eHrs = endTime.getHours() == 0 && newEnd.getDate() != newStart.getDate()
                        ? 24 : endTime.getHours();
            }
            ...
            eHeight = (eHrs - startTime.getHours()) * (rowSize * rowsPerHour);
        ...
        //} else if (rectChanged || canvas.needsResize) {
    ...
    */
    ...
    adjustDimensionsForOverlap : function (canvas, left, top, width, height) {
        ...
        ;
        if (!props) {
            //this.logWarn(canvas.event.name + " - no _overlapProps");
            return;
       }
        var isTimeline = this.isTimelineView(),
    ...
    refreshVisibleEvents : function (events, refreshAll, caller) {
        ...
        if (!this._drawnEvents) {
            this.refreshEvents("refreshVisibleEvents");
            return;
        }
        ...
        for (var i = 0; i < eventsLen; i++) {
            ...
            if (alreadyVisible) {
                ...
                if (this.isGrouped || this.useEventCanvasPool) {
                    ...
                    this.sizeEventCanvas(canvas, true, "refreshVisibleEvents");
                }
                continue;
            }
    ...
    setEventCanvasTabPositions : function () {
        ...
        for (var j=0; j<events.length; j++) {
            ...
            if (canvas && canvas.isEventCanvas && canvas.event) {
                ...
                var tabIndex = 0;
                if (this.hasLanes()) {
                    tabIndex = this.getLaneIndex(canvas.event.lane) * 1000;
                }
                tabIndex += j;
    ...
    getVisibleEvents : function (refreshAll) {
        ...
        for (var i = 0; i < eventsLen; i++) {
            ...
            if (useLanes && cal.shouldShowLane(this.getLane(event.lane), this) == false) continue;
            ...
            if (isWeekView) {
                ...
                if (lStart.getDate() != lEnd.getDate()) {
                    eventEnd = isc.DateUtil.getEndOf(eventStart.duplicate(), "d", false);
                }
    ...
    addEvent : function (event, retag) {
        ...
        if (this.isDayView() && this.hasLanes()) {
        ...
        var canEdit = cal.canEditEvent(event, this);
        if (canvas.setDragProperties) {
        ...
        if (!this.shouldShowColumnLayouts()) {
            ...
            this.addEventCanvasToColumnLayout(canvas);
        }
    },
    ...
    addEventCanvasToColumnLayout : function (eventCanvas) {
...
isc.DaySchedule.addProperties({
    ...
    initWidget : function () {
        ...
        if (this.isDayView() && this.hasLanes() && cal.alternateLaneStyles) {
        ...
        if (lanes.length > 0) {
            ...
            for (var i=0; i<numDays; i++) {
                ...
                var field = {
                    ...
                    showIf: "return list.calendar.showWeekends || !field.isWeekend;",
                ...
                if (this.shouldShowColumnLayouts()) {
                    field.eventLayout = this.columnLayouts[i];
                }
                fields.add(field);
    ...
    updateColumnLayouts : function () {
        ...
        if (this.fields) {
            ...
            for (var i=this.fields.length-1; i>=0; i--) {
                var f = this.fields[i];
                if (f.eventLayout) {
                    f.eventLayout.fieldName = f.name;
                    if (!this.body.contains(f.eventLayout)) {
                        this.body.addChild(f.eventLayout);
                    }
                    f.eventLayout.setLeft(this.getColumnLeft(f.masterIndex));
                    f.eventLayout.setWidth(this.getFieldWidth(f.masterIndex));
                    f.eventLayout.setHeight(this.body.getScrollHeight());
                    if (f.eventLayout.isDrawn()) f.eventLayout.redraw();
                    else f.eventLayout.draw();
                    f.eventLayout.show();
                    f.eventLayout.bringToFront();
                }
            }
        }
    },
    ...
    setFields : function () {
        ...
        if (this.calendar.showColumnLayouts) this.updateColumnLayouts();
        return result;
    },
    ...
    updateRowDates : function () {
        ...
        for (var i=0; i<rowCount; i++) {
            ...
            rec.logicalTime = isc.DateUtil.getLogicalTimeOnly(time);
            rec.logicalDate = isc.DateUtil.getLogicalDateOnly(time);
    ...
    getDateTopOffset : function (date, lane) {
        ...
        ;
        if (col==null) {
            // col doesn't matter in dayView, all cols have the same time-rows
            col = 0;
        }
        for (var i=0; i<len; i++) {
    ...
    getBaseStyle : function (record, rowNum, colNum) {
        ...
        var wStart = isWeek ? this.getDateFromCol(bodyCol) : cal.chosenDate.duplicate(),
            ...
            lane = this.hasLanes() ? this.body.getField(bodyCol)[cal.laneNameField] : null
        ;
    ...
    updateEventWindow : function (event) {
        ...
        for (var i = 0; i < arr.length ; i++) {
            if (arr[i] && arr[i].isEventCanvas && this.areSame(arr[i].event, event)) {
                ...
                // a new object
                arr[i].event = event;
                this.sizeEventCanvas(arr[i], null, "updateEventWindow");
...
isc.MonthSchedule.addProperties({
    ...
    initWidget : function () {
        ...
        // set the range dates
        if (this.rangeUnit) {
            this.periodStartDate = this.startDate = isc.DateUtil.getStartOf(this.chosenDate, this.rangeUnit, false);
            this.periodEndDate = this.endDate = isc.DateUtil.getEndOf(this.chosenDate, this.rangeUnit, false);
        }
    ...
    getDayArray : function () {
        ...
        endDate = new Date(cal.year, cal.month, displayDate.getDate() + 40);
    ...
    cellClick : function (record, rowNum, colNum) {
        ...
        cal.setChosenDate(isc.DateUtil.createLogicalDate(currDate.getFullYear(),
...
isc.TimelineView.addProperties({
    ...
    setChosenDate : function (newDate) {
        ...
        this.setTimelineRange(this.chosenDate, null, null, null, null, this.calendar.headerLevels, true);
    },
                                 ...
                                 timelineUnitsPerColumn, headerLevels, fromSetChosenDate)
    {
        ...
        start = isc.DateUtil.getStartOf(start, granString, false);
    ...
    calcFields : function () {
            ...
            if (cal.laneFields) {
                ...
                for (var i = 0; i < laneFields.length; i++) {
                    var lf = isc.addProperties({}, {
                        ...
                        minWidth: this.labelColumnWidth,
                        width: this.labelColumnWidth,
                        canHover: this.shouldShowLaneFieldHovers(),
                        showHover: this.shouldShowLaneFieldHovers()
                    }, laneFields[i]);
                    newFields.add(lf);
                }
            } else {
        ...
        while (sDate.getTime() <= eDateMillis) {
            ...
            thisDate = isc.DateUtil.getStartOf(thisDate.duplicate(), cal.timelineGranularity, false);
            ...
            if (showDate) {
                ...
                newField = isc.addProperties({}, {
                    ...
                    hoverDelay: this.hoverDelay+1,
                    hoverMoveWithMouse: true,
                    canHover: this.shouldShowCellHovers(),
                    showHover: this.shouldShowCellHovers()
                }, this.getFieldProperties(thisDate));

                if (cal.shouldDisableDate(thisDate)) {
                    newField.dateDisabled = true;
                    newField.baseStyle = "calendarDisabled";
                }
    ...
    getHeaderLevelTitle : function (headerLevel, spanIndex, startDate, endDate) {
        ...
        if (!title) {
            ...
            if (unit == "century" || unit == "decade") {
                ...
                var midWeek = isc.DateUtil.getStartOf(startDate.duplicate(), "W", false, this.calendar.firstDayOfWeek);
    ...
    setSpanDates : function (span, date) {
        ...
        span[cal.startDateField] = isc.DateUtil.getStartOf(date, key, false, cal.firstDayOfWeek);
        span[cal.endDateField] = isc.DateUtil.getEndOf(span[cal.startDateField], key, false, cal.firstDayOfWeek);
    },
    ...
    draw : function (a, b, c, d) {
        ...
        if (this.scrollToToday != false) {
            ...
            var diff = cal.getDayDiff(this.startDate, today);
        ...
        if (this._groupOnDraw) {
            ...
            this.groupBy(this.groupByField);
        }
    ...
    updateEventWindow : function (event) {
        ...
        if (this.renderEventsOnDemand) {
            ...
            for (var i = 0; i < events.length; i++) {
                ...
                this.sizeEventCanvas(canvas, null, "updateEventWindow");
            }
        }
    },
...
isc.DaySchedule.addClassProperties({
    ...
    _getEventScaffolding : function (view) {
        var minsPerRow = view.getTimePerCell(),
            ...
            date = isc.DateUtil.getLogicalDateOnly(view.startDate)
        ;

        //var startTime = view.startTime || isc.DateUtil.createLogicalTime(0, 0),
        //    endTime = view.endTime || isc.DateUtil.createLogicalTime(23, 59, 59, 999)
        //;
        ...
        for (var i=0; i<rowCount; i++) {
            var time = date.duplicate();
            data.add(isc.addProperties({}, row, {
                time: time.duplicate(),
                logicalTime: isc.DateUtil.getLogicalTimeOnly(time),
                logicalDate: isc.DateUtil.getLogicalDateOnly(time)
            }));
            date = isc.DateUtil.dateAdd(date, "mn", minsPerRow, 1);
        }
    ...
    _getCellDates : function (view) {
        ...
        var logicalDate = isc.DateUtil.getLogicalDateOnly(startDate);
        ...
        for (var j=0; j < counter; j++) {
            ...
            for (var i=0; i<rowCount; i++) {
                ...
                var newCellDate = isc.DateUtil.dateAdd(date.duplicate(), "mn", minsToAdd, 1);
...
refreshSelectedView : function () {
    ...
    if (this.dayViewSelected()) {
        this.dayView.refreshEvents("refreshSelectedView");
        ...
        this.weekView.refreshEvents("refreshSelectedView");
        ...
        this.timelineView.refreshEvents("refreshSelectedView");
    }
},
...
getEventHeaderHTML : function (event, view) {
    ...
    var fTime = isc.Time.toShortTime(sDate, this.timeFormatter, false);
    var sTime = view.isTimelineView() ? null : fTime,
...
canEditEvent : function (event, view) {
    ...
    // don't use date.isToday() because it calls getStart/EndOf(), and those methods have been
    // altered to always convert to the current display timezone, which is no good here
    var sDate = isc.DateUtil.getLogicalDateOnly(this.getEventStartDate(event)),
        eDate = isc.DateUtil.getLogicalDateOnly(new Date(this.getEventEndDate(event).getTime()-1)),
        isToday = (isc.DateUtil.compareLogicalDates(sDate, eDate) == 0)
    ;
    if (!view.allowMultiDayEvents && !isToday) {
...
createEventCanvasComponent : function (canvas) {
    ...
    var component = this.createAutoChild("eventCanvasComponent", {
...
columnLayoutDefaults: {
    ...
    drop : function () {
        ...
        var canvas = isc.EventHandler.getDragTarget();
        if (!this.calendar.canEditEvent(canvas.event)) return;
        ...
        var field = this.calendarView.getFieldByName(this.fieldName),
            ...
            newStart = isc.DateUtil.combineLogicalDateAndTime(field.startDate, sTime),
            newEnd = isc.DateUtil.combineLogicalDateAndTime(field.endDate, eTime)
        ;
        var newEvent = cal.createEventObject(canvas.event, newStart, newEnd)
        // run the cancellable eventRepositionStop() (default just checks disabled dates)
        var allowDrop = cal.eventRepositionStop(canvas.event, newEvent, {}, this.calendarView);
        if (allowDrop) this.creator.updateCalendarEvent(canvas.event, newEvent);
    }
},
...
_getEventCanvas : function (event, view) {
    var canDrag = this.canDragEvent(event, view),
        canEdit = this.canEditEvent(event, view),
        canResize = this.canResizeEvent(event, view),
        canRemove = this.canRemoveEvent(event, view),
...
setChosenDate : function (newDate, fromTimelineView) {
    ...
    if (this.timelineView) {
        if (!fromTimelineView) {
            this.timelineView.setChosenDate(this.chosenDate);
        }
    } else {
...
_getTabs : function () {
    ...
    if (this.showDayView != false) {
        this.dayView = this.createAutoChild("dayView",
            isc.addProperties(
                {viewName: "day", chosenDate: this.chosenDate},
            props,
    ...
    if (this.showWeekView != false) {
        this.weekView = this.createAutoChild("weekView",
            isc.addProperties(
                {viewName: "week", chosenDate: this.chosenDate, _refreshEventsOnDraw: true},
            props,
                { cellHeight: this.rowHeight, enforceVClipping: true }
            )
        );
    ...
    if (this.showMonthView != false) {
        this.monthView = this.createAutoChild("monthView",
            isc.addProperties(
                {viewName: "month", chosenDate: this.chosenDate},
            props,
    ...
    if (this.showControlsBar != false) {
        ...
        this.datePickerButton = this.createAutoChild("datePickerButton", {
            click: function () {
                ...
                cal.dateChooser.firstDayOfWeek = cal.firstDayOfWeek;
                ...
                if (showTime != null) cal.dateChooser.showTimeItem = showTime;
                cal.dateChooser.setData(cal.chosenDate, showTime == null);
...
addEventOrUpdateEventFields : function () {
    ...
    if (editStyle == "date" || editStyle == "datetime") {
        ...
        var startMillis = startDate.getTime(),
            maxEndDate = isc.DateUtil.getEndOf(startDate.duplicate(), "D", false)
        ;
...
setDateLabel : function () {
    ...
    var endDate = view.isDayView() ? null : view.endDate;
    var content = this.getDateLabelText(view.viewName, view.startDate, endDate);
...
getDayBodyHTML : function (date, events, calendar, rowNum, colNum) {
    ...
    for (var i = 0; i < moreItemIndex; i++) {
        ...
        if (!this.isPrinting && this.canEditEvent(events[i], view)) {
...
isc.EventWindow.addProperties({
    ...
    click : function () {
        ...
        if (doDefault) {
            if (!cal.canEditEvent(this.event, this.calendarView)) return;
            // handle the case when a selection is made, then an event is clicked
    ...
    parentResized : function () {
        this.Super('parentResized', arguments);
        if (this.calendarView.shouldShowColumnLayouts()) return;
...
isc.TimelineWindow.addProperties({
    ...
    click : function () {
        ...
        ;
        if (doDefault) {
            if (!cal.canEditEvent(this.event, tl)) return;
            cal.showEventDialog(this.event);
...
isc.EventCanvas.addProperties({
    ...
    _updateValueCache : function (event, styleName, headerStyle, bodyStyle) {
        ...
        if (this.event) {
            var cal = this.calendar,
                ...
                canEdit = cal.canEditEvent(event, view),
                canDrag = cal.canDragEvent(event, view),
                canResize = cal.canResizeEvent(event, view),
                canRemove = cal.canRemoveEvent(event, view)
            ;
    ...
    setEventStyle : function (styleName, headerStyle, bodyStyle) {
        ...
        if (this.event) {
            // cater for color settings on the event record itself
            if (!this.isDrawn()) {
                this._updateCanvasColorsOnDraw = true;
            } else {
                this.updateCanvasColors();
            }
        }
    ...
    updateCanvasColors : function () {
        ...
        if (handle) {
            handle["background-color"] = this.event.backgroundColor;
            if (this.event.borderColor) {
                handle["border-style"] = "solid";
                handle["border-color"] = this.event.borderColor;
            } else {
...
// rendering
    ...
    getHeaderCSSText : function () {
        ...
        if (this.getShowBody() == false) {
            // no body, header fills the canvas
            sb.append("bottom:0px; top:0px;");
        } else {
            // header fits content (height auto in the css) and sits top or bottom
    ...
    getBodyCSSText : function (headerHTML) {
        var event = this.event,
            sb = isc.StringBuffer.create(),
            headerHeight = 0
        ;
        ...
        if (headerHTML) {
            headerHeight = isc.Canvas.measureContent(headerHTML, null, true, null,
                { width: this.getInnerWidth() });
        }

        // set the body-top to the header-height (bottom, left and right are already set)
        sb.append("top:", headerHeight, "px; ");
        //if (this.calendarView.shouldShowColumnLayouts()) sb.append("height:auto; ");
...
// generating HTML
    ...
    getInnerHTML : function () {
        ...
        // skip innerHTML is there's a component being shown
        if (this.event && !this.component) {
            var headerHTML = "",
                bodyHTML = "",
            ...
            if (showBody) {
                ...
                bT[3] = this.getBodyCSSText(headerHTML);
                bT[4] = "' eventPart='body'>";
            ...
            if (this.overflow == "visible") {
                // resize the canvas to fit the new content - used when stacking events in a
                // calendar.columnLayout (a VLayout filling a column in vertical views)

                var headerHeight = isc.Canvas.measureContent(headerHTML, null, true, null,
                    { width: this.getInnerWidth() });
                var bodyHeight = isc.Canvas.measureContent(bodyHTML, this.styleName + "Body", true, null,
                    { width: this.getInnerWidth() });
                //this.setHeight(headerHeight + bodyHeight);
                this.setHeight(bodyHeight + this.getVMarginBorder());
            }
        } else {
            if (this.overflow == "visible") {
                // resize the canvas to fit the new content - used when stacking events in a
...
// internal stuff - mouse handler
    ...
    asyncGetPrintHTML : function (printProperties, callback) {
        ...
        if (isTimeline) {
            ...
            top = this.getTop() + headerHeight;

            // add ((eventRow + 1) * 2px)
            var laneIndex = view.getLaneIndex(event[cal.laneNameField]);
            top += ((laneIndex + 1) * 2) - 1;
